<div class="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-shadow cursor-move task-card"
    ...
    <div class="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
        @if(!auth()->user()->isCustomer())
            <div class="flex space-x-2 time-controls" data-time-tracking>
                @if($task->isRunning() && $task->runningTimeEntry && $task->runningTimeEntry->user_id === auth()->id())
                    <button wire:click.stop="stopTimer({{ $task->id }})"
                            class="time-controls text-xs bg-red-100 text-red-700 px-2 py-1 rounded hover:bg-red-200">
                        ⏹ Stop
                    </button>
                @else
                    <button wire:click.stop="startTimer({{ $task->id }})"
                            class="time-controls text-xs bg-green-100 text-green-700 px-2 py-1 rounded hover:bg-green-200">
                        ▶ Start
                    </button>
                @endif

                <button wire:click.stop="openTimeModal({{ $task->id }})"
                        class="time-controls text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded hover:bg-blue-200">
                    + Log
                </button>
            </div>
        @else
    ...
    @if(!auth()->user()->isCustomer())
        ...
        @if($task->timeEntries && $task->timeEntries->count() > 0)
            <div class="mt-2 text-xs text-gray-500 duration-display" data-time-tracking>
