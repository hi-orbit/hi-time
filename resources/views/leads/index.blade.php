    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
            ...
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Manage Leads</h1>
                    <p class="text-gray-600 mt-2">Track potential customers and convert them when proposals are accepted.</p>
                </div>
                            ...
                            <tr class="hover:bg-gray-50">
                                        ...
                                        <div class="flex-shrink-0 h-10 w-10">
                                            <div class="h-10 w-10 rounded-full bg-yellow-100 flex items-center justify-center">
                                                <span class="text-yellow-600 font-medium text-sm">
                                                    {{ strtoupper(substr($lead->display_name, 0, 1)) }}
                                                </span>
                                            </div>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">{{ $lead->display_name }}</div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        ...
                                        <a href="{{ route('proposals.create', ['lead_id' => $lead->id]) }}" class="text-purple-600 hover:text-purple-900" title="Create Proposal">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                            </svg>
                                        </a>
                                        @if($lead->status === 'active')
                                            <form action="{{ route('leads.convert', $lead) }}" method="POST" class="inline" onsubmit="return confirm('Convert this lead to a customer?')">
                                                @csrf
                                                <button type="submit" class="text-green-600 hover:text-green-900" title="Convert to Customer">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
