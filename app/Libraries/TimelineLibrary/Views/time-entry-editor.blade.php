...
<div class="time-entry-editor border border-gray-200 rounded-lg p-4 hover:bg-gray-50 relative z-10 {{ $compactMode ? 'p-2' : '' }}">
    @if($isEditing)
        <!-- Edit Form -->
        <div class="space-y-4 relative z-20 bg-white border border-gray-300 rounded-lg p-4 shadow-lg">
            <div class="grid grid-cols-1 {{ $compactMode ? 'md:grid-cols-2' : 'md:grid-cols-3' }} gap-4">
                <!-- Duration Input -->
                <div>
        ...
        <div class="flex items-start justify-between">
            <div class="flex-1">
                ...
                <div class="mb-2">
                    <div class="flex items-center space-x-2">
                        <span class="text-sm font-medium text-gray-900">{{ $displayTitle }}</span>
                        @if($displaySubtitle)
                            <span class="text-sm text-gray-500">•</span>
                            <span class="text-sm text-gray-600">{{ $displaySubtitle }}</span>
                        @endif
                    </div>
                </div>
                ...
                @if($timeEntry->description)
                    <p class="text-sm text-gray-700 {{ $compactMode ? 'truncate' : '' }}">{{ $timeEntry->description }}</p>
                @endif
            </div>
            ...
            <div class="ml-4 flex items-center space-x-2">
                ...
                @foreach($customActions as $key => $action)
                    <button wire:click="executeCustomAction('{{ $key }}')"
                            class="text-{{ $action['color'] ?? 'gray' }}-600 hover:text-{{ $action['color'] ?? 'gray' }}-500 text-sm font-medium">
                        {{ $action['label'] }}
                    </button>
                @endforeach
            </div>
